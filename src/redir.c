...
static int getdestaddr(int serverfd, int family, struct sockaddr_storage *destaddr)
{
    socklen_t socklen;
    ...
    if (tcp_tproxy) {
        return getsockname(serverfd, (struct sockaddr *)destaddr, &socklen);
    }

    switch (family) {
    case AF_INET:
        return getsockopt(serverfd, SOL_IP, SO_ORIGINAL_DST, destaddr, &socklen);
    case AF_INET6:
        return getsockopt(serverfd, SOL_IPV6, IP6T_SO_ORIGINAL_DST, destaddr, &socklen);
    default:
        errno = EAFNOSUPPORT;
        return -1;
    }
...
int create_and_bind(const char *addr, const char *port, int af, int *fds, int *family, int reuse_port)
{
    ...
    for (rp = result; rp != NULL; rp = rp->ai_next) {
        ...
        if (rp->ai_family == AF_INET6) {
            int v6only = 1;
            setsockopt(listen_sock, IPPROTO_IPV6, IPV6_V6ONLY, &v6only, sizeof(v6only));
        }
...
static void accept_cb(EV_P_ ev_io *w, int revents)
{
    listen_io_ctx_t *io_ctx = (listen_io_ctx_t *)w->data;

    if (io_ctx == NULL || io_ctx->listener == NULL) {
        ERROR("[redir] invalid listener context");
        return;
    }

    listen_ctx_t *listener = io_ctx->listener;
    ...
    if (getdestaddr(serverfd, family, &destaddr) != 0) {
    ...
    for (int i = 0; i < listener->remote_num; i++) {
        if (listener->remote_status == NULL || listener->remote_status[i]) {
            server->remote_idx = i;
            break;
        }
    }
    ...
    metrics_inc_remote_tcp_connections(
        server->remote_idx,
        get_addr_str(listener->remote_addr[server->remote_idx], true));

    if (verbose) {
        LOGI("[redir] tcp: starting remote connect for fd %d", serverfd);
    }
...
static remote_t *new_remote(int fd, int timeout)
{
    ...
    if (remote->recv_ctx == NULL || remote->send_ctx == NULL || remote->buf == NULL) {
        ERROR("[redir] malloc remote context failed");
        if (remote->recv_ctx) ss_free(remote->recv_ctx);
        if (remote->send_ctx) ss_free(remote->send_ctx);
        if (remote->buf) ss_free(remote->buf);
        ss_free(remote);
        return NULL;
    }
    ...
    memset(remote->buf, 0, sizeof(buffer_t));
...
static server_t *new_server(int fd)
{
    ...
    if (server->recv_ctx == NULL || server->send_ctx == NULL || server->buf == NULL) {
        ERROR("[redir] malloc server context failed");
        if (server->recv_ctx) ss_free(server->recv_ctx);
        if (server->send_ctx) ss_free(server->send_ctx);
        if (server->buf) ss_free(server->buf);
        ss_free(server);
        return NULL;
    }
    ...
    memset(server->buf, 0, sizeof(buffer_t));
    ...
    if (server->e_ctx == NULL || server->d_ctx == NULL) {
        ERROR("[redir] malloc cipher ctx failed");
        if (server->e_ctx) ss_free(server->e_ctx);
        if (server->d_ctx) ss_free(server->d_ctx);
        bfree(server->buf);
        ss_free(server->buf);
...
static void server_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    ssize_t r = recv(server->fd, remote->buf->data + remote->buf->len, SOCKET_BUF_SIZE - remote->buf->len, 0);
    if (r <= 0) {
...
static void remote_send_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (!remote->fastopen_sent) {
        ...
        int hlen = construct_relay_header(&server->destaddr, abuf.data);
        if (hlen <= 0) {
            bfree(&abuf);
            handle_tcp_fail(EV_A_ server);
            return;
...
static void remote_timeout_cb(EV_P_ ev_timer *watcher, int revents)
{
    tcp_remote_ctx_t *remote_ctx = cork_container_of(watcher, tcp_remote_ctx_t, watcher);

    if (remote_ctx == NULL) return;
...
static void start_connect_remote(EV_P_ server_t *server)
{
    if (server == NULL || server->listener == NULL) {
        ERROR("[redir] invalid server");
        return;
    }

    listen_ctx_t *listener = server->listener;

    if (server->remote_idx < 0 || server->remote_idx >= listener->remote_num) {
        ERROR("[redir] invalid remote index");
        handle_tcp_fail(EV_A_ server);
        return;
    }

    struct sockaddr *addr = listener->remote_addr[server->remote_idx];
    if (addr == NULL) {
        ERROR("[redir] remote addr NULL");
        handle_tcp_fail(EV_A_ server);
        return;
    }

    socklen_t addr_len = get_sockaddr_len(addr);
    if (addr_len == 0) {
        ERROR("[redir] invalid addr len");
        handle_tcp_fail(EV_A_ server);
        return;
    }
    ...
    remote_t *remote = new_remote(fd, listener->timeout);
    if (remote == NULL) {
        close(fd);
        handle_tcp_fail(EV_A_ server);
        return;
    ...
    memset(&remote->addr_storage, 0, sizeof(remote->addr_storage));
    memcpy(&remote->addr_storage, addr, addr_len);
    remote->addr_len = addr_len;

    if (!fast_open) {
        int r = connect(fd, (struct sockaddr *)&remote->addr_storage, remote->addr_len);

        if (verbose) {
            LOGI("[redir] connect return=%d errno=%d", r, errno);
        }

        if (r < 0 && errno != EINPROGRESS && errno != EWOULDBLOCK) {
            ERROR("[redir] connect remote");
            close_and_free_remote(EV_A_ remote);
            close_and_free_server(EV_A_ server);
            return;
        }
...
static void handle_tcp_fail(EV_P_ server_t *server)
{
    ...
    if (server->listener != NULL && server->remote_idx >= 0 && server->remote_idx < server->listener->remote_num) {
...
int main(int argc, char **argv)
{
    ...
    while ((c = getopt_long(argc, argv, "f:s:p:l:k:t:m:c:b:a:n:huUTv6A:", long_options, NULL)) != -1) {
        switch (c) {
    ...
    if (conf_path != NULL) {
        ...
        if (key == NULL) key = conf->key;
        if (method == NULL) method = conf->method;
        if (timeout == NULL) timeout = conf->timeout;
        if (user == NULL) user = conf->user;
        if (plugin == NULL) plugin = conf->plugin;
        if (plugin_opts == NULL) plugin_opts = conf->plugin_opts;
        if (mode == TCP_ONLY) mode = conf->mode;
        if (tcp_tproxy == 0) tcp_tproxy = conf->tcp_tproxy;
        if (mtu == 0) mtu = conf->mtu;
        if (mptcp == 0) mptcp = conf->mptcp;
        if (no_delay == 0) no_delay = conf->no_delay;
        if (reuse_port == 0) reuse_port = conf->reuse_port;
        if (tcp_incoming_sndbuf == 0) tcp_incoming_sndbuf = conf->tcp_incoming_sndbuf;
        if (tcp_incoming_rcvbuf == 0) tcp_incoming_rcvbuf = conf->tcp_incoming_rcvbuf;
        if (tcp_outgoing_sndbuf == 0) tcp_outgoing_sndbuf = conf->tcp_outgoing_sndbuf;
        if (tcp_outgoing_rcvbuf == 0) tcp_outgoing_rcvbuf = conf->tcp_outgoing_rcvbuf;
        if (fast_open == 0) fast_open = conf->fast_open;
#ifdef HAVE_SETRLIMIT
        if (nofile == 0) nofile = conf->nofile;
#endif
        if (ipv6first == 0) ipv6first = conf->ipv6_first;
        dscp_num = conf->dscp_num;
        dscp = conf->dscp;
        if (fwmark == 0 && conf->fwmark > 0) fwmark = conf->fwmark;
        if (probe_interval == 0) probe_interval = conf->probe_interval;
        if (probe_timeout == 0) probe_timeout = conf->probe_timeout;
        if (probe_up_count == 0) probe_up_count = conf->probe_up_count;
        if (probe_down_count == 0) probe_down_count = conf->probe_down_count;
        if (probe_domain == NULL) probe_domain = conf->probe_domain;
        if (metrics_port == 0) metrics_port = conf->metrics_port;
    }

    if (remote_num == 0 || remote_port == NULL || local_port == NULL || (password == NULL && key == NULL)) {
        usage();
        exit(EXIT_FAILURE);
    }
    ...
    if (plugin != NULL) {
        ...
        if (port == 0) FATAL("[redir] failed to find a free port");
    ...
    if (method == NULL) method = "chacha20-ietf-poly1305";
    if (timeout == NULL) timeout = "600";

#ifdef HAVE_SETRLIMIT
    if (nofile > 1024) {
        if (verbose) LOGI("[redir] setting NOFILE to %d", nofile);
        set_nofile(nofile);
    }
#endif
    ...
    if (pid_flags) daemonize(pid_path);

    if (no_delay) LOGI("[redir] enable TCP no-delay");
    if (ipv6first) LOGI("[redir] resolving hostname to IPv6 address first");
    if (mptcp != 0) LOGI("[redir]enable multipath TCP (%s)", mptcp > 0 ? "out-of-tree" : "upstream");
    if (mtu > 0) LOGI("[redir] set MTU to %d", mtu);
    if (timeout) LOGI("[redir] set timeout to %s", timeout);
    if (fwmark > 0) LOGI("[redir] set fwmark to %d", fwmark);

    if (tcp_incoming_sndbuf != 0 && tcp_incoming_sndbuf < SOCKET_BUF_SIZE) tcp_incoming_sndbuf = 0;
    if (tcp_incoming_sndbuf != 0) LOGI("[redir] set TCP incoming connection send buffer size to %d", tcp_incoming_sndbuf);

    if (tcp_incoming_rcvbuf != 0 && tcp_incoming_rcvbuf < SOCKET_BUF_SIZE) tcp_incoming_rcvbuf = 0;
    if (tcp_incoming_rcvbuf != 0) LOGI("[redir] set TCP incoming connection receive buffer size to %d", tcp_incoming_rcvbuf);

    if (tcp_outgoing_sndbuf != 0 && tcp_outgoing_sndbuf < SOCKET_BUF_SIZE) tcp_outgoing_sndbuf = 0;
    if (tcp_outgoing_sndbuf != 0) LOGI("[redir] set TCP outgoing connection send buffer size to %d", tcp_outgoing_sndbuf);

    if (tcp_outgoing_rcvbuf != 0 && tcp_outgoing_rcvbuf < SOCKET_BUF_SIZE) tcp_outgoing_rcvbuf = 0;
    if (tcp_outgoing_rcvbuf != 0) LOGI("[redir] set TCP outgoing connection receive buffer size to %d", tcp_outgoing_rcvbuf);

    if (probe_interval <= 0) probe_interval = 60;
    if (probe_timeout <= 0) probe_timeout = 5;
    if (probe_up_count <= 0) probe_up_count = 3;
    if (probe_down_count <= 0) probe_down_count = 3;
    if (!probe_domain || strlen(probe_domain) == 0) probe_domain = "www.google.com";
    ...
    if (plugin != NULL) {
        ...
        int err = start_plugin(plugin, plugin_opts, remote_str, remote_port, plugin_host, plugin_port, MODE_CLIENT);
        if (err) FATAL("[redir] failed to start the plugin");
    }
    ...
    if (crypto == NULL) FATAL("[redir] failed to initialize ciphers");

    struct listen_ctx listen_ctx;
    memset(&listen_ctx, 0, sizeof(listen_ctx));

    listen_ctx.remote_num = remote_num;
    listen_ctx.remote_addr = ss_malloc(sizeof(struct sockaddr *) * remote_num);
    if (listen_ctx.remote_addr == NULL) FATAL("[redir] malloc remote addr failed");
    ...
    if (listen_ctx.remote_status == NULL) FATAL("[redir] malloc remote status failed");

    memset((void *)listen_ctx.remote_status, 0, sizeof(bool) * remote_num);
    ...
    for (i = 0; i < remote_num; i++) {
        ...
        if (storage == NULL) FATAL("[redir] malloc sockaddr failed");
    ...
    if (mode != TCP_ONLY) {
        init_udprelay(local_addr, local_port, listen_ctx.remote_num, listen_ctx.remote_addr,
                     mtu, crypto, listen_ctx.timeout, NULL, fwmark, listen_ctx.remote_status);
    }
    ...
    if (mode != UDP_ONLY) {
        ...
        do {
            ...
            if (dscp_num > 0) {
                ...
                if (new_lc == NULL) FATAL("[redir] malloc listen ctx failed");
        ...
        for (i = 0; i < listen_ctx_count; i++) {
            ...
            int fd_count = create_and_bind(local_addr, listener->local_port, AF_UNSPEC, fds, families, reuse_port);

            if (fd_count <= 0) FATAL("[redir] bind failed");

            listener->fd_num = fd_count;

            for (int j = 0; j < fd_count; j++) {
                listener->fd[j] = fds[j];
                listener->family[j] = families[j];

                listener->io_ctx[j].listener = listener;
                listener->io_ctx[j].family = listener->family[j];
                listener->io_ctx[j].fd = fds[j];
